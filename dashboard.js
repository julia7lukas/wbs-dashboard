// ── LIVE DATA — fetched from Jira/Confluence MCP on refresh ──────────
// No hardcoded names, sprint data, or team rosters live here.
// TEAMS is populated by refreshFromJira() via the Anthropic API + Atlassian MCP.
// On first load, any previously saved state (localStorage) is restored.
let TEAMS = {};
...
function showLoadPrompt() {
  document.querySelector('.main').innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:60vh;gap:20px;text-align:center;padding:40px">
      ...
      <div style="font-size:13px;color:var(--t2);max-width:420px">Click <strong style="color:var(--green)">↻ Jira</strong> in the top bar to pull live sprint data from Jira via MCP,<br>or ask Claude: <em>"refresh the WBS sprint dashboard"</em></div>
      <button class="btn btn-g" style="padding:10px 24px;font-size:14px" onclick="refreshFromJira()">↻ Load from Jira</button>
    </div>`;
}
...
function buildTeamSelector() {
  ...
  Object.keys(TEAMS).forEach(k => {
    ...
    b.className = 'btn team-btn'; b.dataset.key = k;
    b.textContent = k; b.style.padding = '4px 10px'; b.style.fontSize = '11px'; b.style.fontWeight = '600';
...
function renderIssues() {
  ...
  // % Complete = done issues / total issues (count-based to match board)
...
// ── ASSIGNED VS AVAILABLE — bar list only, no second chart ─────────
function renderAvail() {
  ...
  // Remove second chart — destroy and hide canvas
  if (charts.cap) { charts.cap.destroy(); charts.cap = null; }
  const cc = document.getElementById('cap-chart');
  if (cc) cc.style.display = 'none';
}

// ── CALENDAR — timezone-safe, reflects teamDays dates exactly ──────
function renderCal() {
  ...
  // Use isoDate() for all comparisons — avoids UTC vs local mismatch
  ...
  while (cur <= e) {
    ...
    const iso = isoDate(cur); // local date string, no UTC shift
    ...
    else if (isWeekend) { /* weekend — no class, just blank */ }
...
// ── BURNDOWN — issue count, decrements from sprint start to end ────
function renderBurndown() {
  const startStr = document.getElementById('start-date').value || getSD().startDate;
  const endStr = document.getElementById('end-date').value || getSD().endDate;
  const s = parseDate(startStr);
  const e = parseDate(endStr);
  if (!s || !e) return;
  ...
  // Build ordered list of working days in the sprint
  const workDays = [];
  const cur = new Date(s);
  while (cur <= e) {
    const dow = cur.getDay();
  ...
  // Ideal: straight line from totalIssues → 0
  const n = workDays.length;
  const ideal = workDays.map((_, i) =>
    Math.round(totalIssues * (1 - i / Math.max(n - 1, 1)) * 10) / 10
  );

  // Actual: today's index in working days
  // Before today: interpolate from totalIssues down to openCount
  // At today: openCount (what's actually remaining right now)
  // After today: null (no data yet)
  ...
  charts.bd = new Chart(document.getElementById('burndown-chart'), {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: ctx => ctx.dataset.label + ': ' + ctx.raw + ' issues remaining' } }
      },
      scales: {
        x: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#5a6580', font: { size: 10 }, maxTicksLimit: 8 } },
        y: {
          grid: { color: 'rgba(255,255,255,.04)' },
          ticks: { color: '#5a6580', font: { size: 10 }, stepSize: 1, precision: 0 },
          min: 0,
          max: Math.max(5, totalIssues),
...
// ── RECALC — % Complete uses issue count to match Jira board ───────
function recalc() {
  const tc = members.reduce((a, m) => a + cap(m), 0);
...
// ── JIRA REFRESH — Anthropic API + Atlassian MCP ──────────────────
async function refreshFromJira() {
  const btn = document.getElementById('refresh-btn');
  if (btn) { btn.textContent = '⏳ Syncing...'; btn.disabled = true; }

  try {
    const keys = currentTeam ? [currentTeam] : ['WBS', 'GIN', 'MOJO', 'MAV', 'AMGO'];

    const prompt = `You are a sprint data agent. Use the Atlassian MCP to pull live sprint data for Jira project keys: ${keys.join(', ')}.

For each project key:
1. Query Jira: project = {KEY} AND sprint in openSprints() AND issuetype not in subTaskIssueTypes() ORDER BY created DESC
   Fields: summary, assignee, status, issuetype, subtasks, aggregatetimeoriginalestimate, aggregatetimespent
2. Pull team roster from the Confluence page titled "{TEAM NAME} Team Roster" in the Payerpath space — parse Name, Role, Hrs/Day columns
3. Get sprint start/end dates from the Jira sprint metadata
...
Return ONLY raw JSON (no markdown fences, no commentary) in this exact shape:
{
  "WBS": {
    "team": "Webslingers",
    "projectKey": "WBS",
    "sprintName": "WBS 2026 Q2S2",
    "startDate": "2026-04-15",
    "endDate": "2026-04-29",
    "workDays": 9,
    "hrsPerDay": 6,
    "syncedAt": "${new Date().toISOString()}",
    "members": [{"name":"Julia Lukas","hrs":6,"pto":0}],
    "teamDays": [{"date":"2026-04-24","type":"Recharge","note":""}],
    "issues": [{"key":"WBS-1","type":"Story","summary":"...","assignee":"Julia Lukas","est":8,"logged":2,"status":"In Progress","subtasks":"WBS-2 🔄"}],
    "subtaskHrs": {"Julia Lukas": {"est": 8, "logged": 2}}
  }
}`;
    ...
    if (start === -1 || end === -1) throw new Error('No JSON returned from MCP');
    const parsed = JSON.parse(clean.slice(start, end + 1));

    Object.assign(TEAMS, parsed);
    localStorage.setItem('wbs-teams-cache', JSON.stringify(TEAMS));
    buildTeamSelector();
    switchTeam(currentTeam || Object.keys(TEAMS)[0]);
    document.getElementById('sync-ts').textContent = 'Jira sync: ' + new Date().toLocaleString([], { dateStyle: 'short', timeStyle: 'short' }) + ' · Live ✓';

  } catch (err) {
    console.error('Jira refresh failed:', err);
    alert('Sync failed: ' + err.message + '\n\nAlternatively: ask Claude "refresh the ' + (currentTeam || 'WBS') + ' sprint dashboard"');
  } finally {
